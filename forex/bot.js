...
//
// v3.0 ELITE MAX UPGRADES (ported from GAS v1.0 + fully rebuilt):
...
const CONFIG = {
  ...
  // Dead zone for ALL forex pairs: 21:00–06:00 UTC
  SESSION_FILTER        : true,
  ...
  // Twelve Data rate limit: 8 req/min on free tier
  // We sleep 1.5s between calls to stay safe
  TD_SLEEP_MS : 1500,
};
...
function detectGWP(candles4h, vp, avwap, math, dec) {
  ...
  for (let age = 0; age <= 2; age++) {
    ...
    // Only stale if the TARGET (VAL midpoint) has already been reached.
    // A signal between entry and target is still alive and actionable.
  ...
  for (const key of posKeys) {
    ...
    if (isL ? price <= p.sl : price >= p.sl) {
      const pathBNote = p.isPathB
        ? `\n⚡ <b>Path B sweep</b> — re-enter near <code>${p.reEntry||"zone"}</code> after sweep.` : "";
