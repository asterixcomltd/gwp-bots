...
// before porting): POC prominence scoring (+5 decisive / -3 contested) and
// POC migration scoring (-4 if migrating with trade direction) added to
// computeConviction; new stocks_equity_curve.json logs every closed trade for
// real peak-equity/max-drawdown reporting in weekly summaries. See
// crypto_bot.js's v4.0 header note for the full rationale — identical logic.
// ════════════════════════════════════════════════════════════════════════════
...
const TF_CONFIG = {
  D1: {
    ...
    atrBufMult:0.50, maxAge:1, avwapProx:0.0025,
  ...
  H4: {
    ...
    minRR:1.5, minConviction:68, cooldownHrs:3,  // v3.5: raised 60 → 68 (backtest: conv 60-69 had 0% WR); cooldown 4h → 3h
  ...
  M30: {
    ...
    atrBufMult:0.62, maxAge:1, avwapProx:0.0045,
...
const CONFIG = {
  ...
  TP3_MULT:2.0,  // v3.5: lowered 3.0 → 2.0 (backtest: 3.0× rarely hit; 2.0× captures more runners)
...
const V = "GWP Stocks v4.0 | Elite Max™ | Asterix Holdings Ltd. | Abdin";
...
// separate from stocks_state.json so weekly reports can compute a real max
// drawdown / peak-equity instead of only ever showing a single running total.
const EQUITY_FILE = path.join(__dirname, "stocks_equity_curve.json");
...
function getEquityStats() {
  try {
    ...
    const chronological = [...curve].reverse();
...
async function fetchYahooCandles(symbol, interval, limit, retry = 0) {
  ...
  const intervalMap = { "15m": "15m", "30m": "30m", "1h": "60m", "1d": "1d" };
  const rangeMap    = {
    "15m": "1mo",  // 1 month of 15m = ~520 bars (enough for Hurst 120+ candles)
    "30m": "2mo",  // 2 months of 30m — Yahoo's practical cap for this granularity
    "60m": "3mo",  // 3 months of 1h = ~780 bars (enough for Hurst via 4H aggregation)
    "1d":  "2y",   // 2 years of daily — covers the D1 vote/entry TF's warmup (v5.0)
  };
...
async function fetchKlines(symbol, tf, limit) {
  ...
  if (tf === "M30") return fetchYahooCandles(symbol, "30m", limit || 100);
...
function computeVolumeProfile(candles, lookback) {
  ...
  if (sl.length >= 20) {
    ...
    const bIdx = (chunk) => { const b = new Array(rows).fill(0); chunk.forEach(c => { for (let r = 0; r < rows; r++) { const rB = lo + r * rowH, rT = rB + rowH, ov = Math.min(c.high, rT) - Math.max(c.low, rB); if (ov > 0) b[r] += (c.vol || 1) * (ov / ((c.high - c.low) || rowH)); } }); let idx = 0; for (let i = 1; i < rows; i++) if (b[i] > b[idx]) idx = i; return idx; };
...
// which itself ports the MVS bot's 5-TF 3-of-5 vote design) ────────────────────
...
async function trackClose(symbol, direction, pnlPct, isWin, convScore = null) {
  ...
  setProp(wk, JSON.stringify(w));
  // v4.0: all-time equity curve (ported from MVS equity-curve.json). Stocks
  // has no compounding/effective-capital engine, so this keeps its own simple
  // all-time cumulative P&L counter, separate from the weekly `w.pnl` reset.
  const allTimeCum = parseFloat((parseFloat(getProp("S1_CUM_PNL") || "0") + p).toFixed(3));
  setProp("S1_CUM_PNL", String(allTimeCum));
  appendEquityPoint(p, allTimeCum);
}
async function sendWeeklyReport() {
  ...
  // v4.0: equity curve summary (ported from MVS)
  const eq = getEquityStats();
  if (eq) msg += `\n📉 <b>Equity Curve:</b> index ${eq.current.toFixed(2)} (peak ${eq.peak.toFixed(2)}, max DD ${eq.maxDrawdownPct}%) · ${eq.points} closes logged\n`;
...
async function runBot() {
  ...
  for (const symbol of CONFIG.PAIRS) {
    try {
      ...
      // v5.0 SPEED: parallel fetch — 1H (for 4H aggregation) + 30M + 15M + D1
      const [c1h_raw, c30m, c15m, cd1] = await Promise.all([
        fetchYahooCandles(symbol, "1h", 500),
        fetchKlines(symbol, "M30", TF_CONFIG.M30.vpLookback + 60),
      ...
      // v5.0: extended from the 3-TF cascade to a full 5-TF, 3-of-5 vote
      // (ported from the MVS bot's validated design, mirrors crypto_bot.js
      // and forex_bot.js). D1 is now one of five voters instead of a
      // separate counter-trend hard block.
      ...
      if (corrBlock) { console.log(`  🔗 Correlation filter: ${symbol} blocked by open ${corrBlock} ${vote.direction}`); continue; }
...
(async () => {
  ...
  // v5.0 fix: the scan cron fires twice per hour (:10/:40), so a bare
  // getUTCHours()===21 check was sending this report twice every Friday.
  // Guarded with a same-day sent-flag, mirroring the startup-message dedup
  ...
  if (!getProp(startKey) && mode === "scan") {
    ...
    await tgSend(
      ...
      `🔥 Engine: 5-TF Vote (1D+4H+1H+30M+15M), 3-of-5 · Vol+AVWAP gate · ATR SL floor\n` +
