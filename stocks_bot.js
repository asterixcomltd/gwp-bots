"use strict";
// ════════════════════════════════════════════════════════════════════════════
// GHOST WICK PROTOCOL — STOCKS EDITION  v3.0  MONEY PRINTING MACHINE ELITE MAX™
...
function getD1Bias(cd1) {
  if (!cd1 || cd1.length < 5) return "NEUTRAL";
  const closes = cd1.map(c => c.close);
  let tv = 0, v = 0;
  cd1.forEach(c => { const tp = (c.high + c.low + c.close) / 3; tv += tp * (c.vol || 1); v += (c.vol || 1); });
...
function computeConviction(gwp, math, ms, tfKey, isConfluence = false, isTriple = false, d1Bias = "NEUTRAL") {
  ...
  // D1 BIAS ALIGNMENT — aligned = +6, counter-trend = -4
...
function detectGWP(candles, vp, avwap, math, tfCfg) {
  ...
  for (let age = 0; age <= tfCfg.maxAge; age++) {
    ...
    const tp1 = direction === "BEAR" ? entry - bH * 1.0 : entry + bH * 1.0;
    ...
    return {
      direction, grade: "GWP", score: score.toFixed(1), rawScore, age,
