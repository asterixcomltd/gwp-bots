...
module.exports = {
  ...
  SYMBOLS: [
    ...
    // v1.1.9 (frequency) — 4 additional liquid crosses. Kept deliberately
    // modest (not doubling the list like crypto) because this bot shares
    // Twelve Data's per-minute/per-day credit budget (see twelvedata.js
    // header) — watch the scan logs after deploying for "daily credit
    // budget exhausted" warnings; trim this list back down if that shows
    // up, or add a dedicated FOREX_TWELVE_DATA_KEYS entry to grow the
    // budget instead of trimming symbols.
    'EUR/GBP', 'AUD/JPY', 'EUR/AUD', 'GBP/CAD',
  ],
